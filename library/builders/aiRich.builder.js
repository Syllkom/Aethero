...
export class AIRich extends BaseBuilder {
    ...
    addImageButton(leftLogoUrl, { rightLogo, rightLogoUrl, width = 100, height = 100, fontHeight = 24, padding = -5, ctaText, ctaUrl, id, replace, insertAt } = {}) {
        const primitives = []
        const rLogo = rightLogo || rightLogoUrl

        if (leftLogoUrl) {
            const keyLeft = 'logo_l_' + crypto.randomBytes(4).toString('hex')
            primitives.push({
                text: `{{${keyLeft}}}.{{/${keyLeft}}}`,
                inline_entities: [
                    {
                        key: keyLeft,
                        metadata: {
        ...
        if (rLogo) {
            const keyRight = 'logo_r_' + crypto.randomBytes(4).toString('hex')
            primitives.push({
                text: `{{${keyRight}}}.{{/${keyRight}}}`,
                inline_entities: [
                    {
                        key: keyRight,
                        metadata: {
        ...
        const submessage = { messageType: 2, messageText: ctaText || 'Action Row' }
        return this._addContent(section, submessage, { id, replace, insertAt })
    }
    ...
    addCompactEntity(entities = [], { id, replace, insertAt } = {}) {
        const items = Array.isArray(entities) ? entities : [entities]
        const sections = items.map(item => AIRich.newLayout('Single', {
            title: item.title || '',
            subtitle: item.subtitle || '',
            secondary_subtitle: item.secondarySubtitle || item.secondary_subtitle || '',
            image: {
                url: Toolkit.resolveMedia(this.#client, item.image || item.image_url || '', 'image'),
                mime_type: 'image/jpeg',
                __typename: 'GenAIMediaItem'
            },
            entity_id: item.id || item.entity_id || Date.now(),
            entity_url: item.url || item.entity_url || '',
            entity_type: (item.type || item.entity_type || 'GROUP').toUpperCase(),
            action_type: (item.action || item.action_type || 'JOIN').toUpperCase(),
            is_verified: item.verified !== undefined ? !!item.verified : false,
            __typename: 'GenAICompactEntityPrimitive'
        }))

        const submessage = { messageType: 2, messageText: 'Compact Entity' }
        return this._addContent(sections, submessage, { id, replace, insertAt })
    }
