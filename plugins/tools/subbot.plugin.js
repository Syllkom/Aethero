...
async function bindSubBotHandlers(subSock, modules) {
    if (!subSock) return
    ...
    subSock.ev.removeAllListeners('messages.upsert')
...
async function startSubBot(numero, sid, isReconnect = false, initiatorChatId = null, modules, mainSock) {
    ...
    subClient.events.on('connection', async (update) => {
        ...
        if (update.type === 'open') {
            const activeSock = subClient.sock
            if (activeSock) {
                await bindSubBotHandlers(activeSock, modules)
                global.subBots.set(sid, { client: subClient, sock: activeSock, startTime: Date.now() })

                if (initiatorChatId) {
                    await mainSock.sendMessage(initiatorChatId, {
                        text: `Sub-bot de @${sid} vinculado y acoplado exitosamente. Estado: Online.`,
        ...
        if (update.type === 'restart' || (update.type === 'error' && update.reasonCode === 428)) {
            ...
            const newSock = await subClient.restart({
                sessionName: sid,
                folderPath: credsPath
            })
            if (newSock) {
                await bindSubBotHandlers(newSock, modules)
                global.subBots.set(sid, { client: subClient, sock: newSock, startTime: Date.now() })
            }
        }
    ...
    const initialSock = await subClient.start({
    ...
    if (initialSock) {
        await bindSubBotHandlers(initialSock, modules)
        global.subBots.set(sid, { client: subClient, sock: initialSock, startTime: Date.now() })
    }
}
    ...
    script: async (m, { sock, modules }) => {
        ...
        if (sub === 'create' || sub === 'vincular') {
            ...
            if (global.subBots.has(sessionId)) {
                ...
                if (item && item.sock) return m.reply('Ya tienes una sesion activa en estado Online. Usa .bot stop antes de crear otra.')
            }
