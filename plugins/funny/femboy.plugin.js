...
export default {
    ...
    case: ['femboy'],
    description: 'Escanea el porcentaje Femboy con overlay temático en Canvas.',
    category: 'funny',
    usage: ['femboy ‹@usuario opcional›'],
    script: async (m, { sock }) => {
        await m.react('wait')
        await initFont()
        ...
        const [ppBuff, overlayBuff] = await Promise.all([
            ...
            downloadBuffer('https://files.catbox.moe/3suzru.png')
        ])
        ...
        if (percent > 88) {
            ...
            ctx.strokeText('¡FEMBOY TOTAL!', 256, 470)
            ctx.fillText('¡FEMBOY TOTAL!', 256, 470)
        }
        ...
        ctx.strokeStyle = '#000000'
        ctx.lineWidth = 12
        ctx.fillStyle = percent > 50 ? '#ec4899' : '#ffffff'
        ...
        const comment = percent > 50 
            ? '🎀 ¿Falda o delirio? La confusión de género puede ser fashion. Sigue intentando, te queda bien.'
            : 'Macho de pelo en pecho. Todavía no te llega la hora de ponerte las medias largas.'
