...
global.config = {
    ...
    saveHistory: true,               // habilita sock.loadMessage()
    autoRead: false,
    silentConsole: true,             // si es true, func.log.handler.js NO imprime nada
    startupNotification: false       // si es true, envía un fakeOrder + adMenu al primer root al conectar
}

global.config.userRoles = {
    "447342719758": {                // clave: número de teléfono O jid completo
        root: true,
        owner: true,
        mod: true,
        vip: true
...
### 6.0. Sistema de botones (`mapButtons`)

Todos los menús interactivos (`orderStatusMenu`, `mediaMenu`, `productMenu`, `interactiveMenu`, `locationMenu`) comparten el mismo array `data.buttons`, transformado internamente por `mapButtons()` (`library/builders/interactiveBuilder.js`). Cada botón es un objeto `{ type, text, ... }`:

| `type` | Botón nativo generado | Parámetros relevantes |
|---|---|---|
| `url` | `cta_url` | `text`, `url` |
| `call` | `cta_call` | `text`, `phone` (o `id`) |
| `copy` | `cta_copy` | `text`, `payload` (código a copiar) |
| `reminder` | `cta_reminder` | `text`, `id` |
| `cancel_reminder` | `cta_cancel_reminder` | `text`, `id` |
| `address` | `address_message` | `text`, `id` |
| `location` | `send_location` | `text`, `id` |
| `vcard` | `vcard_message` | `text`, `vcard` |
| `list` | `single_select` | `text` (título), `sections` |
| `signup` | `inapp_signup` | — |
| `contact` | `request_contact_info` | — |
| `order_status` | `order_status` | `referenceId`, `status`, `price`, `currency` |
| `galaxy` / `flow` | `galaxy_message` | `mode`, `version`, `token`, `flowId`, `text`, `action`, `payload`, `metadata` |
| *(sin `type`)* | `quick_reply` | `text`, `id` |

Cualquier botón admite además `icon` (ícono nativo) e `inline: true` (agrupa varios botones "inline" bajo `has_multiple_buttons`). Si un botón trae `buttonId`/`buttonText` (formato legado de Baileys), se traduce automáticamente a `quick_reply` o al `nativeFlowInfo` explícito si se provee.

Adicionalmente, todos estos menús aceptan:

- **`data.bottomSheet: { limit, title, buttonTitle }`** — convierte el menú en una "hoja inferior" desplegable.
- **`data.offer: { text, url, code, expiration }`** — añade una oferta por tiempo limitado (`limited_time_offer`) sobre el menú.
- **`data.inline: true`** — inserta un botón "fantasma" al inicio y fuerza `forwardingScore: 9999` (aspecto de mensaje reenviado múltiples veces).
- **`options.quoted`** — cita un mensaje (acepta tanto el `m.raw` del framework como un objeto `{ key, message }`).
- **`options.mentions`** — array de JIDs a mencionar.
...
### 6.10. Mensajes de Comercio Directos (`commerceBuilder.js`)
