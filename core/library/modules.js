...
const FileImport = async function (filePath) {
    try {
        ...
        console.error(`Error de importación (${filePath}):`, e.message)
        return null
    }
}

export class ModuleFolder {
    constructor(FolderWatcher, folder, suffix, defaultContext) {
        this.filesJS = new Map()
        this.folder = path.resolve(folder)
        this.store = FolderWatcher.storeManag.getStore(this.folder)
        ...
        this.moduleExport.setFile = (filePath, module) => {
            ...
            for (const key of exportsKeys) this.exports.set(key, module.export[key])
    ...
    import(query) {
        ...
        } else if (query?.file) return this.file(query.file)
        return null
    }
    ...
    export(key, value) {
        ...
        else Object.assign(this.exports.get(key), value)
        return this.exports.get(key)
    }

    query(criteria) {
        if (Array.isArray(criteria) || typeof criteria !== 'object' || criteria === null) return []
        ...
        for (const [relPath, plugin] of this.suffixFiles) {
            ...
            const matches = Entries.every(([key, expected]) => {
                const actual = plugin[key] !== undefined ? plugin[key] : plugin.export?.[key]
    ...
    matchValue(a0, a1) {
        ...
        if (a0 instanceof RegExp) return (typeof a1 === 'string' && a0.test(a1))
        if (Array.isArray(a0) && Array.isArray(a1)) return a0.some(v => a1.includes(v))
        if (typeof a0 === 'string' && Array.isArray(a1)) return a1.includes(a0)
        if (Array.isArray(a0) && typeof a1 === 'string') return a0.includes(a1)
...
export class ModuleRegistry {
    constructor(folders) {
        ...
        this.FolderWatcher = new Watcher(Folders.map(o => o.folder))
        ...
        const safeImport = (filePath) => {
            ...
            const timer = setTimeout(async () => {
                ...
                if (folder) await folder.ImportFile(filePath)
            }, 300)

            this.timers.set(filePath, timer)
        }

        this.FolderWatcher.eventHandler.add = async (filePath) => safeImport(filePath)
        this.FolderWatcher.eventHandler.change = async (filePath) => safeImport(filePath)
    ...
    getFolder(identifier) {
        if (path.isAbsolute(identifier)) return this.foldersMap.get(path.resolve(identifier))
        return Array.from(this.foldersMap.entries()).find(([p]) => path.basename(p) === identifier)?.[1]
    }
